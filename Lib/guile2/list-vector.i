...
%define TYPEMAP_LIST_VECTOR_INPUT_WITH_EXPR(C_TYPE, SCM_TO_C_EXPR, SCM_TYPE)
  ...
  /* input */
	 ...
	 for (i = 0; i<$1; i++) {
	   SCM swig_scm_value = scm_vector_ref($input, scm_from_signed_integer(i));
	   $2[i] = SCM_TO_C_EXPR;
	 }
       }
       else $2 = NULL;
...
%define TYPEMAP_LIST_VECTOR_OUTPUT_WITH_EXPR(C_TYPE, C_TO_SCM_EXPR, SCM_TYPE)
	...
	arguments for Scheme. */
     ...
     {
       $*1_ltype i;
       SCM res = scm_make_vector(scm_from_signed_integer(*$1),
				SCM_BOOL_F);
       for (i = 0; i<*$1; i++) {
	 C_TYPE swig_c_value = (*$2)[i];
	 SCM elt = C_TO_SCM_EXPR;
	 scm_vector_set_x(res, scm_from_signed_integer(i), elt);
       }
...
TYPEMAP_LIST_VECTOR_INPUT_OUTPUT(char *, scm_to_locale_string, scm_from_locale_string, string);
TYPEMAP_LIST_VECTOR_INPUT_OUTPUT(const char *, scm_to_locale_string, scm_from_locale_string, string);
...
%define TYPEMAP_PARALLEL_LIST_VECTOR_INPUT_WITH_EXPR(C_TYPE, SCM_TO_C_EXPR, SCM_TYPE)
  ...
  /* input */
	...
	later change.  */
     ...
     {
       SCM_VALIDATE_LIST($argnum, $input);
       *_global_list_length = scm_length($input);
...
%define TYPEMAP_PARALLEL_LIST_VECTOR_OUTPUT_WITH_EXPR(C_TYPE, C_TO_SCM_EXPR, SCM_TYPE)
  ...
  /* output */
     ...
     {
       int i;
       SCM res = scm_make_vector(scm_from_signed_integer(_global_arraylentemp),
				SCM_BOOL_F);
...
TYPEMAP_PARALLEL_LIST_VECTOR_INPUT_OUTPUT(size_t, scm_to_size_t, scm_from_size_t, integer);
...
TYPEMAP_PARALLEL_LIST_VECTOR_INPUT_OUTPUT(char *, scm_to_locale_string, scm_from_locale_string, string);
TYPEMAP_PARALLEL_LIST_VECTOR_INPUT_OUTPUT(const char *, scm_to_locale_string, scm_from_locale_string, string);
